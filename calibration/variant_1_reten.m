function vf_ = variant_1_reten(t,x,p1,q)
...
k_m_a   = 10^p1(9);  % Maximum degradation rate micro [1/d]
K_m_a   = 10^p1(2);  % Half saturation constant for degradation [µg C/L]
k_m_h   = 10^p1(3);  % Maximum degradation rate micro [1/d]
K_m_h   = 10^p1(10); % Half saturation constant for degradation [µg C/L]
m       = 10^p1(5);  % Maintenance coeficient [1/d]
Y       = 10^p1(6);  % Substrate uptake efficiency of pesticide degraders bacteria A [1]
fcell   = 10^p1(11); % Fraction of the biomass that is filter out from the chemostat [-]
reAT    = 10^p1(12); % Rate of transfer of AT from in and out of the cell [L/d]
reHY    = 10^p1(12); % Rate of transfer of AT from in and out of the cell [L/d]
