...
function [SSE] = run_variant_2_SA(p1)
%% Fixed Information %%
...
try
     warning off
tic
[ty,cu] = ode15s(@variant_2_chemo,t,c,o_opts,p1',q);
 catch ME
     warning off
end
...
tic
[ty1,cu1] = ode15s(@variant_2_chemo,t1,c,o_opts,p1',q);
 catch ME
     warning off
end
...
try
     warning off
tic
[ty2,cu2] = ode15s(@variant_2_chemo,t2,c,o_opts,p1',q);
 catch ME
     warning off
end
...
try
     warning off
tic
[ty3,cu3] = ode15s(@variant_2_chemo,t3,c,o_opts,p1',q);
 catch ME
     warning off
end
...
try
     warning off
tic
[ty4,cu4] = ode15s(@variant_2_chemo,t4,c,o_opts,p1',q);
 catch ME
     warning off
end
...
fcell = 10^p1(11);
        ...
        c(2)    = ATinitR;                   % Lighter AT inside the cell in the system in solution [mmol AT cm-3]
        c(3)    = ATinitR*SN;                % Heavier AT inside the cell in the system in solution [mmol AT cm-3]
        ...
        c(6)    = HYinitR;                % Hydroxyatrazine INSIDE the cell [mmol AT cm-3]
        c(7)    = HYinitR;                % Hydroxyatrazine outisde the cell [mmol AT cm-3]

try
     warning off
tic
[ty5,cu5] = ode15s(@variant_2_reten,t5,c,o_opts,p1',q);
 catch ME
     warning off
end
...
fcell    = 10^p1(7);
        ...
        HY_outc      = vertcat(cu(end,7),cu1(end,7),cu2(end,7),cu3(end,7),cu4(end,7))/C_HY;       % AT lighter
...
%% Calling Data %%
    ...
    MeasData = (vertcat(DAT_C,abs(EF_C),DAT_R,abs(EF_R)));
    Sigma    = (vertcat(DAT_Csd,sd_C,DAT_Rsd,sd_R));
    
%% SSE %%

SSE     = sum(((output1-MeasData)./Sigma).^2)
end